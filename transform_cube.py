...
def create_colored_pointcube(shift=np.array([0, 0, 0]), size=0.5, points_per_face=16, sphere_radius=1):
    faces = [
        (np.array([ 1, 0, 0]), [1.0, 0.0, 0.0]),  # +X
        (np.array([-1, 0, 0]), [1.0, 1.0, 0.0]),  # -X
        (np.array([0,  1, 0]), [0.0, 1.0, 0.0]),  # +Y
        (np.array([0, -1, 0]), [0.0, 1.0, 1.0]),  # -Y
        (np.array([0, 0,  1]), [0.0, 0.0, 1.0]),  # +Z
        (np.array([0, 0, -1]), [1.0, 0.0, 1.0]),  # -Z
    ]
    
    points_per_edge = int(np.sqrt(points_per_face))
    interval = size / points_per_edge
    half = size / 2.
    cube_points = []

    def create_sphere(center, color):
    ...
    for normal, color in faces:
        coords = np.linspace(-half, half, points_per_edge)
        for i in coords:
            for j in coords:
                if abs(normal[0]) == 1: center = [normal[0]*half, i, j]
                elif abs(normal[1]) == 1: center = [i, normal[1]*half, j]
                else: center = [i, j, normal[2]*half]

                cube_point = create_sphere(center+shift, color)
